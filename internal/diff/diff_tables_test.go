...
func TestDiffTables_AddTable(t *testing.T) {
	...
	var addTableOps []Kind
	for _, op := range ctx.ops {
		if op.Kind() == KindAddTable {
			addTableOps = append(addTableOps, op.Kind())
		}
	}
	if len(addTableOps) != 1 {
		t.Fatalf("expected 1 AddTable op, got %d", len(addTableOps))
	}
}
...
func TestDiffTables_DropTable(t *testing.T) {
	...
	var dropTableOps []Kind
	for _, op := range ctx.ops {
		if op.Kind() == KindDropTable {
			dropTableOps = append(dropTableOps, op.Kind())
		}
	}
	if len(dropTableOps) != 1 {
		t.Fatalf("expected 1 DropTable op, got %d", len(dropTableOps))
	}
}
...
func TestSameConstraintContent_DefinitionExcluded(t *testing.T) {
	// Definition field should NOT affect equality
...
func TestSameConstraintSemantics_FKSameRef(t *testing.T) {
	a := &model.Constraint{
		Type: "foreign_key", Columns: []string{"post_id"},
...
func TestSameConstraintSemantics_FKDifferentRefTable(t *testing.T) {
	...
	b := &model.Constraint{
		Type: "foreign_key", Columns: []string{"post_id"},
		RefSchema: "public", RefTable: "articles", RefColumns: []string{"id"},
	}
	if sameConstraintSemantics(a, b) {
...
func TestSameIndexContent_Identical(t *testing.T) {
	a := &model.Index{Name: "idx_users_email", Unique: true, Method: "btree", WhereClause: "",
		Elements: []model.IndexElem{{Name: "email", IndexColName: "email", Collation: "C", Opclass: "text_ops", Ordering: "ASC", NullsOrdering: ""}}}
	b := &model.Index{Name: "idx_users_email", Unique: true, Method: "btree", WhereClause: "",
		Elements: []model.IndexElem{{Name: "email", IndexColName: "email", Collation: "C", Opclass: "text_ops", Ordering: "ASC", NullsOrdering: ""}}}
...
func TestDiffTableConstraints_ChangeConstraint(t *testing.T) {
	...
	source.Constraints["fk_posts"] = &model.Constraint{
		Name: "fk_posts", Type: "foreign_key", Columns: []string{"post_id"},
		RefSchema: "public", RefTable: "posts", RefColumns: []string{"id"},
	}
	target := model.NewTable("public", "users")
...
// ─── diffTableColumns (rename detection) ──────────────────────
...
func TestSameStringSlice_Equal(t *testing.T) {
	a := []string{"a", "b", "c"}
	b := []string{"a", "b", "c"}
	if !sameStringSlice(a, b) {
		t.Error("expected sameStringSlice to be true")
	}
}

func TestSameStringSlice_DifferentLength(t *testing.T) {
	a := []string{"a", "b"}
	b := []string{"a", "b", "c"}
	if sameStringSlice(a, b) {
		t.Error("expected sameStringSlice to be false for different lengths")
	}
}

func TestSameStringSlice_DifferentContent(t *testing.T) {
	a := []string{"a", "b"}
	b := []string{"a", "c"}
	if sameStringSlice(a, b) {
		t.Error("expected sameStringSlice to be false for different content")
	}
}

func TestSameStringSlice_BothNil(t *testing.T) {
	if !sameStringSlice(nil, nil) {
		t.Error("expected sameStringSlice(nil, nil) to be true")
	}
}

// ─── diffSchemas (schema-level) ────────────────────────────────
...
// ─── ObjectKey equality (used in DependsOn) ────────────────────
